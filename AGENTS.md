...
- MOTHER 초기 등록은 활성 임신 프로필 생성으로 완료하며, 산모당 활성 임신 프로필은 1개만 허용한다.
- FAMILY 초기 등록은 유효한 가족 초대 코드를 수락해 `CONNECTED` 관계가 생성되어야 완료된다.
- 가족 초대는 DB에 저장되는 6글자 초대 코드 방식으로 구현한다.
- 초대 코드는 pregnancy당 활성 코드 1개만 유지하고, 만료 전 여러 가족이 사용할 수 있다.
- 만료된 초대 코드는 요청 시 `EXPIRED`로 저장하고, 초대 발급·수락의 UNIQUE 충돌은 실패한 트랜잭션 종료 후 재시도 또는 재조회한다.
